	...
	if asset and (liability or equity):
		...
		for period in period_list:
			...
			if liability:
				...
				effective_liability += flt(liability[-2][period.key2])
			if equity:
				#effective_liability += flt(equity[-2][period.key])
				effective_liability += flt(equity[-2][period.key2])

			#provisional_profit_loss[period.key] = flt(asset[-2][period.key]) - effective_liability
			provisional_profit_loss[period.key2] = flt(asset[-2][period.key2]) - effective_liability
			if provisional_profit_loss[period.key2]:
				has_value = True
...
def get_period_list(fiscal_year, periodicity, from_beginning=False):
	...
	if periodicity == "Yearly":
		period_list = [_dict({"to_date": end_date, "key": fiscal_year, "label": fiscal_year, "key2":'Total',})]
	else:
		months_to_add = {
			"Half-yearly": 6,
			"Quarterly": 3,
...
def prepare_data(accounts, balance_must_be, period_list):
	...
	for d in accounts:
		...
		for period in period_list:
			if d.get(period.key2):
				# change sign based on Debit or Credit, since calculation is done using (debit - credit)
				# d[period.key] *= (1 if balance_must_be=="Debit" else -1)

				d[period.key2] *= (1 if balance_must_be=="Debit" else -1)
			# row[period.key] = flt(d.get(period.key, 0.0), 3)
			row[period.key2] = flt(d.get(period.key2, 0.0), 3)

			if  abs(row[period.key2]):
				# ignore zero values
				has_value = True
...
def add_total_row(out, balance_must_be, period_list):
	...
	for period in period_list:
		...
		row[period.key2] = out[0].get(period.key2, 0.0)
		out[0][period.key2] = ""
