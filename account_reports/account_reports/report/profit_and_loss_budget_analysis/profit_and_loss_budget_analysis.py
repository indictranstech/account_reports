...
def get_gross_profit(first,second):
	row = ['Gross Profit']
	period_data =[0,0,0,0]
	period_data=[x - y for x, y in zip(first, second)]
	row+=period_data
	return row
...
def get_net_profit_details(first,fourth):
	row = ['Net Profit/Loss']
	period_data =[0,0,0,0]
	period_data=[x - y for x, y in zip(first, fourth)]
	row+=period_data
	return row	
...
def get_data_for_goods_sold(filters,root_type):
	costcenter_target_details= frappe.db.sql("""select cc.name, cc.distribution_id,
		cc.parent_cost_center, bd.account, bd.budget_allocated,bd.root_type
		from `tabCost Center` cc, `tabBudget Detail` bd
		where bd.parent=cc.name and bd.fiscal_year=%s and bd.root_type=%s and bd.account='Cost of Goods Sold - F' and
		cc.company=%s order by cc.name""" % ('%s', '%s' ,'%s'),
		(filters.get("fiscal_year"), root_type, filters.get("company")), as_dict=1)
